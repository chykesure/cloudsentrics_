...
// Send payment token email (optional)
const sendPaymentEmail = (userEmail, paymentToken) => {
  ...
  const mailOptions = {
    ...
    text: `Thank you for your payment. Here is your token: ${paymentToken}`,
  };
...
app.post('/create-payment-intent', async (req, res) => {
  ...
  try {
    ...
    sendPaymentEmail(email, paymentIntent.client_secret);
...
// Stripe Webhook to capture success and insert into DB
app.post('/webhook', express.raw({ type: 'application/json' }), (req, res) => {
  ...
  if (event.type === 'payment_intent.succeeded') {
    ...
    res.status(200).send(); // Respond to other events
  }
});
