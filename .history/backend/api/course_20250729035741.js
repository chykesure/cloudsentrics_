...
router.post('/', async (req, res) => {
  const { fullName, email, phone, country, course, paymentMethod, referral } = req.body;


  if (!fullName || !email || !phone || !country || !course || !paymentMethod || !referral) {
    return res.status(400).json({ error: 'All fields are required' });
  }


  const user = { fullName, email, phone, course, referral };
  ...
  const userMailOptions = {
    ...
    html: `
      <div style="font-family: Arial, sans-serif; padding: 20px; max-width: 600px; margin: auto; color: #333;">
        ...
        <p style="font-size: 16px;">Thank you for enrolling in the <strong>${course}</strong> course with CloudSentrics. Your payment has been received successfully!</p>
  ...
  try {
    ...
    res.json({
      message: 'Course Payment received and confirmation email sent!',
      student: user,
    });
  } catch (error) {
    console.error('❌ Error sending confirmation email:', error);
    res.status(500).json({
      error: 'Course Payment saved, but failed to send confirmation email',
      student: user,
    });
  }
});
