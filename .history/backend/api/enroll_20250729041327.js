...
router.post('/', async (req, res) => {
  const { fullName, email, phone, country, course, paymentMethod, referralName } = req.body;

  if (!fullName || !email || !phone || !country || !course || !paymentMethod || !referralName ) {
    return res.status(400).json({ error: 'All fields are required' });
  }

  const user = { fullName, email, phone, course,  referralName};
  ...
  const userMailOptions = {
    ...
    to: email,
    subject: `🎉 Course Registration Received - ${course}`,
    html: `
      <div style="font-family: Arial, sans-serif; padding: 20px; max-width: 600px; margin: auto; color: #333;">
        ...
        <p style="font-size: 16px;">Thank you for registering for the <strong>${course}</strong> course with CloudSentrics!</p>
        ...
        <div style="margin-top: 20px; font-size: 16px;">
          ...
          <p><strong>🙋‍♂️ Referred By:</strong> ${referralName || 'N/A'}</p>
