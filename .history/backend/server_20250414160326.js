...
app.post('/verify-token', (req, res) => {
    ...
    const query = 'SELECT * FROM students WHERE registration_token = ? AND email = ?';
    ...
    db.query(query, [token, email], (err, results) => {
      ...
      if (results.length > 0) {
        return res.status(200).json({ valid: true, student: results[0] });
      } else {
        return res.status(404).json({ valid: false, message: 'Token not found or email mismatch' });
      }
    });
  });
