...
app.post('/create-payment-intent', async (req, res) => {
  const { amount, email } = req.body;  // Email from the frontend (user making the payment)
...
app.post('/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
  ...
  if (event.type === 'payment_intent.succeeded') {
    ...
    // After successful payment, you can insert the student into the database
    const {
      full_name,
      phone,
      country,
      ...
      registration_paid,
      course_paid,
    } = paymentIntent.metadata;  // Retrieve any metadata you sent along with the payment
    ...
    const values = [
      full_name,
      studentEmail,  // Use email from the payment intent
      phone,
      country,
      course,
      payment_method,
