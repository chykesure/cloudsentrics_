...
const CourseForm = ({ onClose }) => {
    ...
    const handleChange = (e) => {
        ...
        if (name === 'paymentMethod' && value === 'one-time') {
            setFormData((prev) => ({
                ...prev,
                [name]: value,
                amount: 3000, // Set default amount for one-time
            }));
        } else {
            setFormData((prev) => ({
                ...prev,
    ...
    const handleVerifyToken = async () => {
        ...
        setLoading(true);
        try {
            const res = await fetch('https://api.cloudsentrics.org/api/verifytoken', {
                method: 'POST',
                headers: {
    ...
    const handlePaymentSubmit = async (e) => {
        ...
        try {
            const res = await fetch('https://api.cloudsentrics.org/api/create-payment-intent', {
                method: 'POST',
            ...
            if (result.paymentIntent.status === 'succeeded') {
                ...
                const enrollResponse = await fetch('https://api.cloudsentrics.org/api/course', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                console.log('Course data sent successfully:', data); */
    ...
    const getCurrencyInfo = (country, amountUSD) => {
        const rates = {
            Nigeria: { currency: 'NGN', rate: 1400 },
            "United Kingdom": { currency: 'GBP', rate: 0.76 },
            Canada: { currency: 'CAD', rate: 1.35 },
            "United States": { currency: 'USD', rate: 1 },
            Germany: { currency: 'EUR', rate: 0.91 },
            France: { currency: 'EUR', rate: 0.91 },
            India: { currency: 'INR', rate: 83 },
            Australia: { currency: 'AUD', rate: 1.5 },
            "South Africa": { currency: 'ZAR', rate: 18.5 },
            Ghana: { currency: 'GHS', rate: 12 },
            Kenya: { currency: 'KES', rate: 145 },
            China: { currency: 'CNY', rate: 7.2 },
            Brazil: { currency: 'BRL', rate: 5.1 },
            Japan: { currency: 'JPY', rate: 158 },
            Mexico: { currency: 'MXN', rate: 17.2 },
            Netherlands: { currency: 'EUR', rate: 0.91 },
            UAE: { currency: 'AED', rate: 3.67 },
            Singapore: { currency: 'SGD', rate: 1.35 },
            Sweden: { currency: 'SEK', rate: 10.5 },
            Italy: { currency: 'EUR', rate: 0.91 },
            Spain: { currency: 'EUR', rate: 0.91 },
            Pakistan: { currency: 'PKR', rate: 277 },
            Bangladesh: { currency: 'BDT', rate: 110 },
            Russia: { currency: 'RUB', rate: 89 },
            Philippines: { currency: 'PHP', rate: 56 },
        };
        ...
        const converted = (amountUSD * info.rate).toLocaleString(undefined, {
            maximumFractionDigits: 2,
        });

        return { currency: info.currency, amount: converted };
    };
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            ...
                            <div>
                                ...
                                >
                                    ...
                                    {[
                                        "Nigeria", "United States", "United Kingdom", "Canada", "Germany", "France", "India",
                                        "Australia", "South Africa", "Ghana", "Kenya", "China", "Brazil", "Japan", "Mexico", "Netherlands", "UAE", "Singapore", "Sweden", "Italy", "Spain", "Pakistan", "Bangladesh", "Russia", "Philippines"
                                    ].map((country) => (
                            ...
                            <div className="mb-6">
                                <label className="block text-sm font-medium text-gray-600 mb-2">Referral Name (optional)</label>
                                <input
                                    type="text"
                                    name="referral"
