...
const CourseForm = ({ onClose }) => {
    ...
    const handleVerifyToken = async () => {
        ...
        setLoading(true);
try {
    // Check if the email and token are defined
    if (!email || !token) {
    ...
    const res = await fetch('http://localhost:5000/api/verifytoken', {
        ...
        body: JSON.stringify({ email, token }), // Ensure the body is properly structured
    });
    ...
    if (!res.ok) {
        ...
        // Check if the server response contains the 'valid' field
        if (data && data.valid) {
            alert("✅ Token verified successfully!");
            setStep(1); // Continue to the next step if the token is valid
        } else {
