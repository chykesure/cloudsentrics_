...
const CourseForm = ({ onClose }) => {
    ...
    const [courseFee, setCourseFee] = useState(3000); // Default fee for installment payments
    const [totalPaid, setTotalPaid] = useState(0); // Track total paid
    const [paymentCount, setPaymentCount] = useState(0); // Track number of payments made
    ...
    const handlePaymentSubmit = async (e) => {
        ...
        // Validate installment payment logic
        if (formData.paymentMethod === 'installment') {
            if (paymentCount >= 3) {
                alert("❌ You can only make up to 3 payments.");
                return;
            }

            if (paymentCount === 0 && totalPaid < 1000) {
                alert("❌ The first installment must be at least $1000.");
                return;
            }

            const remainingAmount = courseFee - totalPaid;
            if (remainingAmount <= 0) {
                alert("❌ You have already paid the full amount.");
                return;
            }
        }
        ...
        try {
            ...
            if (result.error) {
                ...
                setTotalPaid(prev => prev + result.paymentIntent.amount_received / 100); // Increment total paid in USD
                alert("✅ Payment Successful!");
                if (totalPaid + result.paymentIntent.amount_received / 100 >= courseFee) {
                    alert("🎉 Payment Complete! You've paid the full amount.");
            ...
            <div className="bg-white w-full max-w-3xl p-8 rounded-lg shadow-xl relative overflow-auto max-h-[90vh]">
                ...
                {step === 3 && (
                    <>
                        ...
                        <div className="mb-6">
                            <label className="block text-sm font-medium text-gray-600 mb-2">Amount to Pay: ${courseFee}</label>
                            <div className="w-full p-3 border border-gray-300 rounded-md">
                                <CardElement />
                            </div>
                        </div>
                        <button className="w-full bg-blue-600 text-white py-3 rounded-md hover:bg-blue-700" onClick={handlePaymentSubmit}>
                            Pay Now
                        </button>
                    </>
                )}
