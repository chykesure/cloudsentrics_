...
const CloudHeader = () => {
  ...
  const renderSolutionSubMenu = (isMobile = false) => {
    ...
    // Mobile: completely flat list — no hierarchy / grouping
    return (
      <div className="space-y-3 mt-4 px-2">
        {solutionItems.flatMap((item) => {
          const items = [];

          // Industry name – standalone item
          items.push(
            <a
              key={`${item.industry}-industry`}
              href={item.href}
              onClick={handleSubmenuClick}
              className="block bg-slate-800/85 rounded-xl p-5 hover:bg-slate-700/90 transition border border-slate-700/50 shadow-sm"
              style={fontStyle}
            >
          ...
          // Use case phrase – also completely standalone (same style)
          if (item.useCase) {
            items.push(
              <a
