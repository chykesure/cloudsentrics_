...
const CloudHeader = () => {
  ...
  // NEW: State to "lock" a dropdown open when clicked
  ...
  const handleMenuClick = (e, label) => {
    ...
    // Toggle logic for locking/unlocking dropdowns
    if (lockedDropdown === label) {
      setLockedDropdown(null); // Unlock if already open
    } else {
      setLockedDropdown(label); // Lock the clicked menu
    }
  };
        ...
        <nav className="hidden md:flex items-center space-x-8 relative">
          {navLinks.map((link) => (
            ...
            >
                ...
                {link.submenu && (
                  <FaChevronDown 
                    className={`ml-1 text-xs transition-transform ${
                      // Rotate arrow if dropdown is locked or hovered
              ...
              {/* Dropdown Submenu */}
              {/* LOGIC CHANGE: Show if hovered OR if locked via click */}
                    ...
                    {link.submenu.map((sub, idx) => (
                      <React.Fragment key={sub.label}>
                        <motion.a
                          ...
                          // STYLING CHANGE: White background on hover, dark text on hover
                          className={`block px-5 py-3 text-sm transition-colors duration-150 ${
                            currentPath === sub.href
                              ? 'text-[#04D1FF] bg-blue-900/40' // Active style
                              : 'text-slate-300 hover:bg-white hover:text-slate-900' // White Hover Style
                          }`}
                          whileHover={{ x: 4 }}
                        >
                          {sub.label}
            ...
            <div className="px-4 py-6 flex flex-col space-y-4">
              {navLinks.map((link) => (
                <div key={link.label}>
                  ...
                  {/* Mobile Submenu (Always expanded if active for mobile simplicity, or you can add toggle logic) */}
                  {link.submenu && (activeDropdown === link.label || lockedDropdown === link.label) && (
                    <div className="ml-6 mt-3 space-y-3 border-l-2 border-slate-700 pl-4">
                      {link.submenu.map((sub) => (
                        <motion.a
                          ...
                          // Mobile styling: White hover
                          className={`block text-base transition-colors ${
                            currentPath === sub.href
                              ? 'text-[#04D1FF]'
                              : 'text-slate-400 hover:bg-white hover:text-slate-900 px-2 rounded' // Added padding/rounded for better mobile touch target
                          }`}
                          whileHover={{ x: 4 }}
                        >
                          {sub.label}
