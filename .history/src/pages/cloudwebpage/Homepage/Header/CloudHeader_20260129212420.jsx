  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-slate-900/90 backdrop-blur-md border-b border-slate-800">
      <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
        ...
        {/* Logo */}
        <img src="/cloud.png" alt="Cloud Sentrics Logo" className="h-14 w-auto" />
                ...
                <AnimatePresence>
                  <motion.div
                    ...
                    className={`absolute left-0 top-full mt-3 ${
                      link.label === 'Solution' ? 'w-[560px]' : 'w-64'
                    } bg-white rounded-xl shadow-xl p-6 z-50`}
                  >

                    {/* SOLUTION MENU */}
                    {link.label === 'Solution' ? (
                      <table className="w-full border-0 border-collapse text-sm">
                        <thead>
                          <tr>
                            <th className="text-left pb-3 font-semibold text-slate-800 border-0">
                              Industries
                            </th>
                            <th className="text-left pb-3 font-semibold text-slate-800 border-0">
                              Use Case
                        ...
                        <tbody>
                          {solutionItems.map((item) => {
                            ...
                            return (
                              <tr key={item.industry} className="border-0">
                                {/* Industry */}
                                <td className="py-2 pr-6 border-0 align-top">
                                  <a
                                    href={item.href}
                                    onClick={() => handleSubmenuClick(link.label)}
                                    className={`relative block px-3 py-2 rounded-md transition overflow-hidden
                                      ${
                                        active
                                ...
                                {/* Use Case */}
                                <td className="py-2 border-0 align-top">
                                  {item.useCase && (
                                    <a
                                      href={item.href}
                                      onClick={() => handleSubmenuClick(link.label)}
                      ...
                      link.submenu.map((sub) => (
                        <a
                          ...
                          href={sub.href}
                          onClick={() => handleSubmenuClick(link.label)}
                          className="block px-4 py-3 rounded hover:bg-[#07154C] hover:text-white"
                        >
                          {sub.label}
                        </a>
                      ))
        ...
        {/* CTA */}
        <button className="hidden sm:block bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg">
          Get started
        </button>
