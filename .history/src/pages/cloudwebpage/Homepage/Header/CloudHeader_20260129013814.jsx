...
const CloudHeader = () => {
  ...
  const navLinks = [
    ...
    },
    {
      label: 'Solution',
      href: '#solution',
      submenu: [], // We override rendering for Solution → no need for items here
    },
    {
      label: 'Resources',
      href: '#resources',
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-slate-900/90 backdrop-blur-md border-b border-slate-800">
                    ...
                    {link.label === 'Solution' ? (
                      <table className="w-full border-collapse">
                        ...
                        <tbody className="divide-y divide-slate-100">
                          <tr>
                            <td className="py-2 pr-8 text-slate-700">Education</td>
                            <td className="py-2 text-slate-700">Secure Storage and File Sharing</td>
                          </tr>
                          <tr className="bg-[#07154C] text-white font-medium">
                            <td className="py-2 pr-8">Hospitals</td>
                            <td className="py-2">NDPC Compliance</td>
                          </tr>
                          <tr>
                            <td className="py-2 pr-8 text-slate-700">Laboratories & Diagnostics Centre</td>
                            <td className="py-2 text-slate-700">NDPC Compliance</td>
                          </tr>
                          <tr>
                            <td className="py-2 pr-8 text-slate-700">Law Firms</td>
                            <td className="py-2 text-slate-700"></td>
                          </tr>
                          <tr>
                            <td className="py-2 pr-8 text-slate-700">Manufacturing</td>
                            <td className="py-2 text-slate-700"></td>
                          </tr>
                          <tr>
                            <td className="py-2 pr-8 text-slate-700">Real Estate</td>
                            <td className="py-2 text-slate-700"></td>
                          </tr>
                          <tr>
                            <td className="py-2 pr-8 text-slate-700">Financial Institutions</td>
                            <td className="py-2 text-slate-700">NDPC Compliance</td>
                          </tr>
                          <tr>
                            <td className="py-2 pr-8 text-slate-700">Other Industries</td>
                            <td className="py-2 text-slate-700"></td>
                          </tr>
                        </tbody>
                      </table>
      ...
      {/* Mobile Menu – kept as simple list (table not suitable for mobile) */}
      <AnimatePresence>
        {isOpen && (
          <motion.nav
                ...
                <div key={link.label}>
                  <motion.a
                    ...
                    onClick={(e) => {
                      ...
                      if (!link.submenu?.length) setIsOpen(false);
                    }}
                  ...
                  >
                    {link.label}
                    {link.submenu?.length > 0 && <FaChevronDown className="text-sm" />}
                  </motion.a>

                  {link.submenu?.length > 0 && (activeDropdown === link.label || lockedDropdown === link.label) && (
