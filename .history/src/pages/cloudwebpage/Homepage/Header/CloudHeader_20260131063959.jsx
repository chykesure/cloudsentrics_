...
<tbody className="text-slate-700">
  {solutionItems.map((item) => {
    ...
    return (
      <tr key={item.industry} className="group">
        <td className="py-2 pr-4">
          <a
            href={item.href}
            onClick={handleSubmenuClick}
            className={`block px-3 py-1.5 rounded transition-colors duration-150
              ${active 
                ? 'bg-[#04D1FF] text-white font-medium' 
                : 'group-hover:bg-[#07154C] group-hover:text-white'
              }`}
          >
            {item.industry}
          </a>
        ...
        <td className="py-2">
          {item.useCase ? (
            ...
            // Empty cell — keeps layout stable, no visible content
