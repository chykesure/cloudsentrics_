          ...
          {navLinks.map((link) => (
            ...
            >
              <motion.a
                ...
                className={`flex items-center text-base font-medium transition-all duration-200 ${isActive(link)
                    ? activeColorClass
              ...
              >
                ...
                {link.submenu && (
                  <FaChevronDown
                    className={`ml-1 text-xs transition-transform ${(activeDropdown === link.label || lockedDropdown === link.label)
                        ? 'rotate-180'
              ...
              {link.submenu && (activeDropdown === link.label || lockedDropdown === link.label) && (
                <AnimatePresence>
                  <motion.div
                    ...
                    className="absolute left-0 top-full mt-3 w-64 bg-white border border-slate-200 rounded-xl shadow-2xl py-2 overflow-hidden z-50"
                  >
                    {link.submenu.map((sub, idx) => (
                      <React.Fragment key={sub.label}>
                        <motion.a
                          ...
                          // UPDATED: Explicitly added hover:bg-gray-200 for a visible Gray effect
                          className={`block px-5 py-3 text-sm transition-colors duration-150 ${currentPath === sub.href
                              ? 'bg-[#04D1FF] text-white font-medium'
                              : 'text-slate-700 hover:bg-red-50 hover:text-green-700'
                            }`}
                          whileHover={{ x: 4 }}
                        >
                          {sub.label}
              ...
              {navLinks.map((link) => (
                <div key={link.label}>
                  <motion.a
                    ...
                    className={`flex items-center justify-between text-lg font-medium transition-colors ${isActive(link)
                        ? activeColorClass
                        : `${inactiveColorClass} ${hoverColorClass}`
                      }`}
                    ...
                    <div className="ml-6 mt-3 space-y-3 border-l-2 border-slate-700 pl-4">
                      {link.submenu.map((sub) => (
                        <motion.a
                          ...
                          // UPDATED: Explicitly added hover:bg-gray-800 for a visible Gray effect
                          className={`block text-base transition-colors ${currentPath === sub.href
                              ? 'text-[#04D1FF]'
