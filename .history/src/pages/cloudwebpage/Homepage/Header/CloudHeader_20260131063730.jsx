...
const CloudHeader = () => {
  ...
  const solutionItems = [
    ...
    { industry: 'Laboratories & Diagnostics Centre', href: '/solutions/laboratories' },
        ...
        <table className="w-full border-collapse text-sm">
          ...
          <tbody>
            {solutionItems.map((item) => {
              ...
              const isHighlighted = item.industry === 'Hospitals'; // ← static highlight like screenshot (remove if not wanted)

              return (
                <tr key={item.industry} className={isHighlighted ? 'bg-[#07154C]' : ''}>
                  <td className="py-2 pr-4">
                    <a
                      href={item.href}
                      onClick={handleSubmenuClick}
                      className={`block px-3 py-1.5 rounded transition text-sm font-medium ${active
                          ? 'bg-[#04D1FF] text-white'
                          : isHighlighted
                            ? 'text-white'
                            : 'text-slate-800 hover:bg-[#07154C] hover:text-white'
                        }`}
                    >
                      {item.industry}
                    </a>
                  ...
                  <td className="py-2">
                    {item.useCase ? (
                      <a
                        href={item.href}
                        onClick={handleSubmenuClick}
                        className={`block px-3 py-1.5 rounded transition text-sm ${active
                            ? 'bg-[#04D1FF] text-white'
                            : isHighlighted
                              ? 'text-white'
                              : 'text-slate-600 hover:bg-[#07154C] hover:text-white'
                          }`}
                      >
                        {item.useCase}
                      </a>
                    ) : (
                      <div className="px-3 py-1.5 text-sm text-slate-400 italic">&nbsp;</div>
                    )}
                  </td>
                </tr>
              );
                  ...
                  link.submenu && (
                    <AnimatePresence>
                      <motion.div
                        ...
                        exit={{ opacity: 0, y: 10 }}
                        className={`absolute left-0 top-full mt-3 ${link.label === 'Solution' ? 'w-[520px]' : 'w-64'
              ...
              {navLinks.map((link) => (
                <div key={link.label}>
                  ...
                  >
                    ...
                    {link.submenu && (
                      <FaChevronDown
                        className={`text-xs transition-transform ${mobileSubMenu === link.label ? 'rotate-180' : ''
                          }`}
                      />
                    )}
                  </button>
