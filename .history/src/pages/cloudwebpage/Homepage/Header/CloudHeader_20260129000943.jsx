        ...
        <nav className="hidden md:flex items-center space-x-8 relative">
          {navLinks.map((link) => (
            ...
            >
              <motion.a
                href={link.href}
                className={`flex items-center text-base font-medium text-slate-300 transition-all duration-200 ${hoverColorClass}`}
                  ...
                  >
                    {link.submenu.map((sub, idx) => (
                      <React.Fragment key={sub.label}>
                        <motion.a
                          href={sub.href}
                          className={`block px-5 py-3 text-sm text-slate-300 hover:bg-blue-900/60 ${submenuHoverColorClass} transition-colors duration-150`}
                          whileHover={{ x: 4 }}
                        >
                          {sub.label}
                        </motion.a>
            ...
            <div className="px-4 py-6 flex flex-col space-y-4">
              {navLinks.map((link) => (
                <div key={link.label}>
                  <motion.a
                    href={link.href}
                    className={`flex items-center justify-between text-lg font-medium text-slate-300 transition-colors ${mobileHoverColorClass}`}
                    ...
                    <div className="ml-6 mt-3 space-y-3 border-l-2 border-slate-700 pl-4">
                      {link.submenu.map((sub) => (
                        <motion.a
                          key={sub.label}
                          href={sub.href}
                          className={`block text-base text-slate-400 ${submenuHoverColorClass} transition-colors`}
