              ...
              {link.submenu && (activeDropdown === link.label || lockedDropdown === link.label) && (
                <AnimatePresence>
                  <motion.div
                    ...
                    // UPDATED: White Background, Light Border, Strong Shadow
                    ...
                    {link.submenu.map((sub, idx) => (
                      <React.Fragment key={sub.label}>
                        <motion.a
                          ...
                          // Hover: Cyan bg with White text
                          // Active: Cyan bg with White text
                          className={`block px-5 py-3 text-sm transition-colors duration-150 ${
                            currentPath === sub.href
                              ? 'bg-[#04D1FF] text-white' // Active State
                              : 'text-slate-700 hover:bg-[#04D1FF] hover:text-white' // Hover State
                          }`}
                          whileHover={{ x: 4 }}
                        >
                          {sub.label}
                        ...
                        {idx < link.submenu.length - 1 && (
                          // Divider color changed to light gray for white bg
                    ...
                    <div className="ml-6 mt-3 space-y-3 border-l-2 border-slate-700 pl-4">
                      {link.submenu.map((sub) => (
                        <motion.a
                          ...
                          // Mobile styling: Keeps dark bg but adds white hover
