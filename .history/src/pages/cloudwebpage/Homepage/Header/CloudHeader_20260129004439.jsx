...
// Import useLocation if you need it for submenus later, but not strictly needed for top-level logic now
...
const CloudHeader = () => {
  ...
  // 1. NEW STATE: Manually track which menu is active. Default is 'Home'.
  ...
  // 2. UPDATED LOGIC: Check the activeMenu state, not the URL
  const isActive = (link) => {
    return activeMenu === link.label;
  };

  // Handler for top-level menu clicks (No Redirect)
  const handleMenuClick = (e, label) => {
    e.preventDefault(); // This stops the page from navigating
    setActiveMenu(label); // This updates the color
  };

  // Handler for submenu clicks (Updates the parent color, but allows navigation)
        ...
        <nav className="hidden md:flex items-center space-x-8 relative">
          {navLinks.map((link) => (
            ...
            >
              <motion.a
                href={link.href}
                // 3. Add onClick handler to prevent navigation and change color
                  ...
                  >
                    {link.submenu.map((sub, idx) => (
                      <React.Fragment key={sub.label}>
                        <motion.a
                          href={sub.href}
                          // 4. When clicking a submenu, update the parent's active state
            ...
            <div className="px-4 py-6 flex flex-col space-y-4">
              {navLinks.map((link) => (
                <div key={link.label}>
                  <motion.a
                    href={link.href}
                    // 5. Add onClick handler for mobile menu items too
                    onClick={(e) => {
                    ...
                    <div className="ml-6 mt-3 space-y-3 border-l-2 border-slate-700 pl-4">
                      {link.submenu.map((sub) => (
                        <motion.a
                          key={sub.label}
                          href={sub.href}
                          // 6. Add onClick handler for mobile submenu items
                          onClick={() => {
