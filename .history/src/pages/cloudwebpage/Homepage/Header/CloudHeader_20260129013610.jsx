...
const CloudHeader = () => {
  ...
  const navLinks = [
    ...
    },
    {
      label: 'Solution',
      href: '#solution',
      submenu: [ // Keep for consistency, but we'll override rendering below
  ...
  // Table data for Solution submenu (matches your screenshot exactly)
  const solutionTableData = [
    { industry: 'Education', useCase: 'Secure Storage and File Sharing' },
    { industry: 'Hospitals', useCase: 'NDPC Compliance', highlight: true },
    { industry: 'Laboratories & Diagnostics Centre', useCase: 'NDPC Compliance' },
    { industry: 'Law Firms', useCase: '' },
    { industry: 'Manufacturing', useCase: '' },
    { industry: 'Real Estate', useCase: '' },
    { industry: 'Financial Institutions', useCase: '' },
    { industry: 'Other Industries', useCase: '' },
  ];
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-slate-900/90 backdrop-blur-md border-b border-slate-800">
                ...
                <AnimatePresence>
                  <motion.div
                    ...
                    className={`absolute left-0 top-full mt-3 ${
                      link.label === 'Solution' ? 'w-96' : 'w-64'
                    } bg-white border border-slate-200 rounded-xl shadow-2xl py-2 overflow-hidden z-50`}
                  >
                    {link.label === 'Solution' ? (
                      // Custom table rendering for Solution submenu (exact match to your screenshot)
                      <div className="px-4 py-3">
                        <table className="w-full border-collapse text-sm">
                          <thead>
                            <tr className="border-b border-slate-200">
                              <th className="text-left font-semibold text-slate-800 pb-2">Industries</th>
                              <th className="text-left font-semibold text-slate-800 pb-2">Use Case</th>
                            </tr>
                          </thead>
                          <tbody>
                            {solutionTableData.map((row, idx) => (
                              <tr
                                key={idx}
                                className={`hover:bg-slate-50 transition-colors ${
                                  row.highlight ? 'bg-[#07154C] text-white font-medium hover:bg-[#07154C]/90' : ''
                                }`}
                              >
                                <td className="py-2 pr-4 border-b border-slate-100">{row.industry}</td>
                                <td className="py-2 border-b border-slate-100">{row.useCase}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                        {/* Optional footer links (ties back to your original submenu) */}
                        <div className="mt-4 pt-3 border-t border-slate-200 text-xs text-slate-500">
                          <a href="/solution/storage" className="hover:text-[#04D1FF] mr-4">Secure Storage</a>
                          <a href="/solution/sharing" className="hover:text-[#04D1FF] mr-4">File Sharing</a>
                          <a href="/solution/compliance" className="hover:text-[#04D1FF]">Compliance Tools</a>
                        </div>
                      </div>
                    ) : (
                      // Standard submenu list for other menus
      ...
      {/* Mobile Menu (unchanged – table too wide for mobile; keeps simple list) */}
      <AnimatePresence>
        {isOpen && (
          <motion.nav
            ...
            <div className="px-4 py-6 flex flex-col space-y-4">
              {navLinks.map((link) => (
                <div key={link.label}>
                  ...
                  {/* Mobile Submenu (simple list – no table for mobile) */}
                  {link.submenu && (activeDropdown === link.label || lockedDropdown === link.label) && (
