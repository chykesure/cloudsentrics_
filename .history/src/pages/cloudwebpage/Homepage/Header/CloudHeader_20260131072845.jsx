...
const CloudHeader = () => {
  ...
  const renderSolutionSubMenu = (isMobile = false) => {
    ...
    // Mobile: completely separate items for industry and use case
    return (
      <div className="space-y-2.5 mt-3 px-1">
        {solutionItems.flatMap((item) => {
          const items = [];

          // Industry - always its own item
          items.push(
            <a
              key={`${item.industry}-industry`}
              href={item.href}
              onClick={handleSubmenuClick}
              className="block bg-slate-800/90 rounded-xl p-4.5 hover:bg-slate-700/90 transition border border-slate-700/60"
              style={fontStyle}
            >
              <div className="text-white font-semibold text-[16.5px]">
                {item.industry}
              </div>
            </a>
          );

          // Use case - separate item when it exists
          if (item.useCase) {
            items.push(
              <a
                key={`${item.industry}-usecase`}
                href={item.href}
                onClick={handleSubmenuClick}
                className="block bg-slate-800/80 rounded-xl p-4.5 hover:bg-slate-700/85 transition border border-slate-700/50 ml-5"
                style={fontStyle}
              >
                <div className="text-slate-200 font-medium text-[15.5px] pl-3 border-l-2 border-[#04D1FF]/50">
                  {item.useCase}
                </div>
              </a>
            );
      ...
      <div className="space-y-1 mt-2">
        {submenu.map((sub) => (
          <a
            ...
            href={sub.href}
            onClick={handleSubmenuClick}
            className="block px-5 py-3.5 rounded-lg hover:bg-slate-800/70 transition text-[15.5px]"
            style={fontStyle}
          >
            {sub.label}
          </a>
