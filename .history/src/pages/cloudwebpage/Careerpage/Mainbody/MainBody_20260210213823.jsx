...
const CheckItem = ({ children }: { children: React.ReactNode }) => (
  <div className="flex items-start gap-3">
    ...
    <p className="text-[#1f1f1f] text-[17px] leading-[28px]">{children}</p>
  </div>
);
...
const CloudSentricsFeatures = () => {
  ...
  // Handle native close (Esc / backdrop click)
  useEffect(() => {
    const dialog = dialogRef.current;
    if (!dialog) return;

    const handleClose = () => setIsNoVacancyModalOpen(false);
    dialog.addEventListener("close", handleClose);
    return () => dialog.removeEventListener("close", handleClose);
  }, []);

  // If you still need accordion functionality – otherwise remove these
      ...
      <section className="bg-[#0f172a] text-white py-20 lg:py-28 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 xl:px-16 relative z-10">
          ...
          <div className="grid md:grid-cols-2 gap-8 lg:gap-12">
            {/* Card 1: View All Vacancies → opens modal */}
        ...
        <dialog
          ref={dialogRef}
          id="no-vacancy-modal"
          className="modal p-0 bg-transparent backdrop:bg-black/70"
          onCancel={(e) => {
            e.preventDefault(); // optional: prevent Esc close if you want
          }}
        >
          <div
            className={`
          ...
          >
            <div className="
              ...
              w-full max-w-md sm:max-w-lg rounded-2xl shadow-2xl border border-slate-200/70
