...
const CloudSentricsFeatures = () => {
  ...
  // Sync state with dialog show/close
  useEffect(() => {
    const dialog = dialogRef.current;
    if (!dialog) return;

    if (isNoVacancyModalOpen) {
      dialog.showModal();           // Opens modal + traps focus
    } else {
      dialog.close();               // Closes cleanly
    }
  }, [isNoVacancyModalOpen]);

  // Optional: Listen for native close (Esc / backdrop click)
  useEffect(() => {
    const dialog = dialogRef.current;
    if (!dialog) return;

    const handleClose = () => {
      setIsNoVacancyModalOpen(false);
    };
