...
const CloudSentricsFeatures = () => {
  ...
  <div className="relative max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
    ...
    {/* Video Container */}
    <div className="relative mx-auto max-w-5xl aspect-video rounded-2xl sm:rounded-3xl overflow-hidden shadow-2xl shadow-blue-900/50 border border-blue-800/40 bg-black">
      {/* Gradient overlay for better play button contrast & premium feel */}
      ...
      <button
        type="button"
        className="absolute inset-0 z-20 flex items-center justify-center bg-black/20 transition-all duration-500 group hover:bg-black/40 focus:outline-none focus:ring-4 focus:ring-blue-500/30"
        onClick={() => {
          const video = document.getElementById('featured-video');
          if (video) {
            video.play();
            // Optional: hide button after play starts
            video.addEventListener('play', () => {
              video.nextElementSibling?.classList.add('opacity-0', 'pointer-events-none');
            }, { once: true });
          }
        }}
        aria-label="Play featured video"
      >
        <div className="relative flex items-center justify-center w-20 h-20 sm:w-28 sm:h-28 md:w-32 md:h-32 lg:w-40 lg:h-40 rounded-full bg-black/50 backdrop-blur-md border-4 border-white/40 shadow-xl group-hover:scale-110 group-hover:bg-blue-700/70 transition-all duration-400">
          <svg
            className="w-10 h-10 sm:w-14 sm:h-14 md:w-16 md:h-16 text-white ml-1"
            fill="currentColor"
            viewBox="0 0 24 24"
          >
    ...
    <div className="mt-12 md:mt-16 text-center max-w-4xl mx-auto">
      ...
      <p className="mt-6 text-base sm:text-lg md:text-xl opacity-80">
        The segment highlighted our mission to empower organizations with enterprise-grade cloud technologies — enabling secure handling of sensitive information, enhanced regulatory compliance, and improved operational efficiency. We are proud to support businesses and institutions with scalable, secure, and locally compliant data management solutions.
      </p>

      <p className="mt-6 text-lg md:text-xl font-medium text-blue-300">
        Watch the full feature to discover our vision, impact, and contribution to Africa's secure digital transformation journey.
      </p>
    </div>
  </div>
</section>
