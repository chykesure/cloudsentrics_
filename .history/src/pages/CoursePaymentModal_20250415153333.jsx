...
const coursePricing = 3000; // Fixed price for all courses
    ...
    const handlePaymentSubmit = async (e) => {
        ...
        try {
            const res = await fetch('http://localhost:5000/create-payment-intent', {
                ...
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    amount: formData.paymentMethod === 'one-time' ? coursePricing : formData.amount * 100, // Multiply by 100 for cents in USD
            ...
            <div className="bg-white w-full max-w-3xl p-8 rounded-lg shadow-xl relative overflow-auto max-h-[90vh]">
                ...
                {step === 1 && (
                    <>
                        ...
                        <button className="w-full bg-[#1e4272] text-white py-3 rounded-md hover:bg-green-700" onClick={handleSubmitStep1}>
                            Next
                        </button>
                    </>
                )}
                ...
                {step === 2 && (
                    <>
                        <h2 className="text-2xl font-semibold mb-6 text-gray-800 text-center">Step 2: Choose Course & Payment</h2>

                        {/* Course and Payment Method Inputs */}
                        <div className="mb-4">
                        ...
                        <div className="mb-4">
                            ...
                            >
                                ...
                                <option value="one-time">One-Time Payment (₦3000)</option>
                                <option value="installment">Installment</option>
                            </select>
                        </div>

                        {/* Optional: Custom Amount Field */}
                        {formData.paymentMethod === 'installment' && (
                            <div className="mb-4">
                                <label className="block text-sm font-medium text-gray-700">Amount (Min ₦1000)</label>
                                <input
                                    type="number"
                                    name="amount"
                        ...
                        <button
                            className="w-full bg-[#1e4272] text-white py-3 rounded-md hover:bg-blue-700 mt-4"
                            onClick={() => {
                                if (isAmountValid()) {
                                    handleSubmitStep2();
                                } else {
                ...
                {step === 4 && (
                    <div className="text-center">
                        <h3 className="text-2xl font-semibold text-gray-800">Thank you for your payment!</h3>
...
const CoursePaymentModal = ({ isOpen, onClose }) => {
